 ...
 * 速尔（Sure）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=SURE）：
 * https://www.kdniao.com/api-track ；速尔开放平台 open.suery.cn 未检索到公开 API 文档，
 * 查询需寄/收件人手机号后四位（CustomerName）。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟
 * 公开协议实现，生产环境签名与 CustomerName 取值需实网验证。
 */
final class Sure implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'SURE';

    /** 速尔状态关键词 => 统一状态（以 AcceptStation 内容匹配，具体在前） */
    ...
    {
        $endpoint = (string) $this->config->get('sure.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('sure.ebusiness_id');
        $appKey = (string) $this->config->get('sure.app_key');
        $customerName = (string) $this->config->get('sure.customer_name', '');
        ...
        if ($customerName !== '') {
            $content['CustomerName'] = $customerName; // 速尔需要寄/收件人手机号后四位
        }
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[SURE %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[SURE %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[SURE] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('SURE createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('SURE createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('SURE subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[SURE] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[SURE] %s', $message));
    }
}
