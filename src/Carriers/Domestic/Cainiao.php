 ...
 * 菜鸟速递（CAINIAO）适配器。
 *
 * VERIFIED-REQUIRED: 契约基于菜鸟速递开放平台公开文档（EDI 轨迹查询接口 TMS_PRACTICE_TRACE_INFO：
 * form 表单提交 logistic_provider_id/logistics_interface/data_digest，
 * data_digest = Base64(Md5(logistics_interface + secretKey))），
 * 生产端点（edi.xpm.cainiao.com）与响应字段名需实网验证。
 */
final class Cainiao implements CarrierInterface
{
    private const ENDPOINT = 'https://edi.xpm.cainiao.com/ext/gateway/ediPracticeTrace/ediStandardPracticeTrace/api';

    /** 菜鸟速递状态关键词 => 统一状态（以 desc 内容匹配） */
    ...
    {
        $providerId = (string) $this->config->get('cainiao.logistic_provider_id');
        $secretKey = (string) $this->config->get('cainiao.secret_key');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[CAINIAO %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[CAINIAO %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[CAINIAO] 响应解析失败');
        }
        ...
        $traces = $result['logisticsTrajectories'] ?? $result['data'] ?? [];
    ...
    {
        throw new LogisticsException('CAINIAO createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('CAINIAO createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('CAINIAO subscribe 待实现');
    }
    ...
    private function throwForApiError(string $code, string $message): never
    {
        if (in_array($code, ['SIGN_CHECK_FAIL', 'sign-check-failure', 'INVALID_SIGN', 'AUTH_FAILED', 'AUTH_ERROR'], true)) {
            throw new AuthException(sprintf('[CAINIAO %s] %s', $code, $message));
        }

        throw new LogisticsException(sprintf('[CAINIAO %s] %s', $code, $message));
    }
}
