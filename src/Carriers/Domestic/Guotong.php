...
/**
 * 国通快递（已停运的加盟快递网络，快递鸟编码 GTO）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=GTO）：
 * https://www.kdniao.com/api-track ；国通快递官网已关闭，未检索到公开开放 API，
 * 仅历史单号可通过第三方聚合平台查询。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现，ShipperCode=GTO 经快递鸟官方编码表确认（https://www.kdniao.com/message/detail/116110），
 * 状态文案映射与单号格式需实网验证（国通单号多为 12 位纯数字，与 uc 等位数规则冲突，
 * 建议显式调用）。
 */
final class Guotong implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'GTO';

    /** 国通快递状态关键词 => 统一状态（以 AcceptStation 内容匹配，具体在前） */
    ...
    {
        $endpoint = (string) $this->config->get('guotong.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('guotong.ebusiness_id');
        $appKey = (string) $this->config->get('guotong.app_key');
        $customerName = (string) $this->config->get('guotong.customer_name', '');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[GUOTONG %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[GUOTONG %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[GUOTONG] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('guotong createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('guotong createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('guotong subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[GUOTONG] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[GUOTONG] %s', $message));
    }
}
