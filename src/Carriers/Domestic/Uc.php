 ...
 * 优速（UC）适配器。
 *
 * VERIFIED-REQUIRED: 官网（uce.cn）未检索到公开开放平台 API（仅第三方聚合平台以编码 UC 接入）；
 * 契约基于行业 EDI 模式（partnerId/timestamp/sign MD5 签名 JSON 接口），端点与轨迹字段名需实网验证。
 */
final class Uc implements CarrierInterface
{
    private const ENDPOINT = 'https://open.uce.cn/api/track/query';

    /** 优速状态关键词 => 统一状态（以 remark 内容匹配） */
    ...
    {
        $partnerId = (string) $this->config->get('uc.partner_id');
        $token = (string) $this->config->get('uc.token');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[UC %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[UC %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[UC] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('UC createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('UC createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('UC subscribe 待实现');
    }
    ...
    private function throwForApiError(string $code, string $message): never
    {
        if (in_array($code, ['401', '403', '10001', '10002', '10003'], true)) {
            throw new AuthException(sprintf('[UC %s] %s', $code, $message));
        }

        throw new LogisticsException(sprintf('[UC %s] %s', $code, $message));
    }
}
