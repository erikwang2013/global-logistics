 ...
 * 中铁物流（快递鸟编码 ZTWL）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=ZTWL）：
 * https://www.kdniao.com/api-track ；中铁物流未检索到公开开放 API，仅可通过第三方聚合平台查询。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现，ShipperCode=ZTWL 与状态文案映射需实网验证（区别于中铁快运 CRE；单号格式未检索到公开资料，建议显式调用）。
 */
final class Zhongtie implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'ZTWL';
    ...
    {
        $endpoint = (string) $this->config->get('zhongtie.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('zhongtie.ebusiness_id');
        $appKey = (string) $this->config->get('zhongtie.app_key');
        $customerName = (string) $this->config->get('zhongtie.customer_name', '');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[ZHONGTIE %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[ZHONGTIE %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[ZHONGTIE] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('zhongtie createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('zhongtie createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('zhongtie subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[ZHONGTIE] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[ZHONGTIE] %s', $message));
    }
}
