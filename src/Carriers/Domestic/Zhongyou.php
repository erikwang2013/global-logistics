...
/**
 * 中邮物流（中国邮政旗下物流品牌，快递鸟编码 ZYWL）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=ZYWL）：
 * https://www.kdniao.com/api-track ；中邮物流官网（11183.com.cn）未检索到公开开放 API，
 * 仅可通过第三方聚合平台查询。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现，ShipperCode=ZYWL 经快递鸟官方编码表确认（https://www.kdniao.com/message/detail/127695
 * 与 https://www.kdniao.com/message/detail/116110），状态文案映射与单号格式需实网验证
 * （中邮物流单号无公开统一格式，建议显式调用）。
 */
final class Zhongyou implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'ZYWL';

    /** 中邮物流状态关键词 => 统一状态（以 AcceptStation 内容匹配，具体在前） */
    ...
    {
        $endpoint = (string) $this->config->get('zhongyou.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('zhongyou.ebusiness_id');
        $appKey = (string) $this->config->get('zhongyou.app_key');
        $customerName = (string) $this->config->get('zhongyou.customer_name', '');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[ZHONGYOU %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[ZHONGYOU %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[ZHONGYOU] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('zhongyou createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('zhongyou createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('zhongyou subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[ZHONGYOU] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[ZHONGYOU] %s', $message));
    }
}
