 ...
 * 壹米滴答（YMD）适配器。
 *
 * VERIFIED-REQUIRED: 官网（yimidida.com）未检索到公开开放平台 API（仅第三方聚合平台接入）；
 * 契约基于行业 EDI 模式（partnerId/timestamp/sign MD5 签名 JSON 接口），端点与轨迹字段名需实网验证。
 */
final class Ymd implements CarrierInterface
{
    private const ENDPOINT = 'https://open.yimidida.com/api/track/query';

    /** 壹米滴答状态关键词 => 统一状态（以 remark 内容匹配；快运以 货物 表述） */
    private const STATUS_MAP = [
        '签收' => TrackStatus::DELIVERED,
        '派送' => TrackStatus::OUT_FOR_DELIVERY,
        '揽收|收件|提货' => TrackStatus::PENDING,
    ...
    {
        $partnerId = (string) $this->config->get('ymd.partner_id');
        $token = (string) $this->config->get('ymd.token');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[YMD %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[YMD %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[YMD] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('YMD createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('YMD createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('YMD subscribe 待实现');
    }
    ...
    private function throwForApiError(string $code, string $message): never
    {
        if (in_array($code, ['401', '403', '10001', '10002', '10003'], true)) {
            throw new AuthException(sprintf('[YMD %s] %s', $code, $message));
        }

        throw new LogisticsException(sprintf('[YMD %s] %s', $code, $message));
    }
}
