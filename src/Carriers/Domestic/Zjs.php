 ...
 * 宅急送（ZJS）适配器。
 *
 * VERIFIED-REQUIRED: 官网（zjs.com.cn）仅对协议客户提供 EDI 标准接口，未检索到公开开放平台 API；
 * 契约基于行业 EDI 模式（partnerId/timestamp/sign MD5 签名 JSON 接口），端点与轨迹字段名需实网验证。
 */
final class Zjs implements CarrierInterface
{
    private const ENDPOINT = 'https://open.zjs.com.cn/api/track/query';

    /** 宅急送状态关键词 => 统一状态（以 remark 内容匹配） */
    ...
    {
        $partnerId = (string) $this->config->get('zjs.partner_id');
        $token = (string) $this->config->get('zjs.token');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[ZJS %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[ZJS %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[ZJS] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('ZJS createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('ZJS createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('ZJS subscribe 待实现');
    }
    ...
    private function throwForApiError(string $code, string $message): never
    {
        if (in_array($code, ['401', '403', '10001', '10002', '10003'], true)) {
            throw new AuthException(sprintf('[ZJS %s] %s', $code, $message));
        }

        throw new LogisticsException(sprintf('[ZJS %s] %s', $code, $message));
    }
}
