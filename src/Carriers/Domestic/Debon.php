 ...
 * 德邦快递（DEBON）适配器。
 *
 * VERIFIED-REQUIRED: 契约基于公开文档模式，需实网验证（端点 https://api.deppon.com/track/query、appKey/appSecret MD5 签名参数形态与轨迹字段名）。
 */
final class Debon implements CarrierInterface
{
    private const ENDPOINT = 'https://api.deppon.com/track/query';

    /** 德邦状态关键词 => 统一状态（以 operation_desc 内容匹配，`|` 分隔同义关键词） */
    ...
    private const TIME_FIELD = 'operation_time';
    private const LOCATION_FIELD = 'city_name';
    private const DESC_FIELD = 'operation_desc';
    ...
    {
        $appKey = (string) $this->config->get('debon.app_key');
        $appSecret = (string) $this->config->get('debon.app_secret');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[DEBON %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[DEBON %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[DEBON] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('DEBON createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('DEBON createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('DEBON subscribe 待实现');
    }
    ...
    private function throwForApiError(string $code, string $message): never
    {
        if (in_array($code, ['401', '403', '10001', '10002', '10003'], true)) {
            throw new AuthException(sprintf('[DEBON %s] %s', $code, $message));
        }

        throw new LogisticsException(sprintf('[DEBON %s] %s', $code, $message));
    }
}
