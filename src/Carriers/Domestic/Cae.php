...
/**
 * 民航快递（中国民航总局下属航空快递，快递鸟编码 CAE）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=CAE）：
 * https://www.kdniao.com/api-track ；民航快递官网 cae.com.cn 仅有网页版快件追踪
 * （https://www.cae.com.cn/webFunction/ExpressQuery/Default.aspx），未检索到开放 API 文档，
 * 以快递鸟聚合协议为准。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现；ShipperCode=CAE 为快递鸟编码表取值，单号多为 CA 开头或纯数字（CA 开头与菜鸟
 * CA 规则冲突，建议显式调用），生产环境是否需 CustomerName 需实网验证。
 */
final class Cae implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'CAE';

    /** 民航快递状态关键词 => 统一状态（以 AcceptStation 内容匹配，具体在前） */
    ...
    {
        $endpoint = (string) $this->config->get('cae.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('cae.ebusiness_id');
        $appKey = (string) $this->config->get('cae.app_key');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[CAE %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[CAE %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[CAE] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('cae createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('cae createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('cae subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[CAE] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[CAE] %s', $message));
    }
}
