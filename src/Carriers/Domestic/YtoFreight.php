...
/**
 * 圆通快运（圆通速递旗下大件快运，区别于圆通速递 YTO，快递鸟编码 YUANTONGKUAIYUN）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=YUANTONGKUAIYUN）：
 * https://www.kdniao.com/api-track ；圆通开放平台未检索到快运轨迹公开 API 文档，
 * 以快递鸟聚合协议为准。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现；ShipperCode=YUANTONGKUAIYUN 为快递鸟编码表取值，单号与圆通速递形态相近
 * （YT 前缀或纯数字，与 yto/yd 规则可能冲突，建议显式调用），生产环境是否需 CustomerName
 * 需实网验证。
 */
final class YtoFreight implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'YUANTONGKUAIYUN';

    /** 圆通快运状态关键词 => 统一状态（以 AcceptStation 内容匹配，具体在前） */
    ...
    {
        $endpoint = (string) $this->config->get('yto-freight.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('yto-freight.ebusiness_id');
        $appKey = (string) $this->config->get('yto-freight.app_key');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[YTO-FREIGHT %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[YTO-FREIGHT %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[YTO-FREIGHT] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('yto-freight createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('yto-freight createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('yto-freight subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[YTO-FREIGHT] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[YTO-FREIGHT] %s', $message));
    }
}
