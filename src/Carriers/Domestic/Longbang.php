...
/**
 * 龙邦速递（快递鸟编码 LONGBANWULIU）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=LONGBANWULIU）：
 * https://www.kdniao.com/api-track ；龙邦速递未检索到公开开放 API，仅可通过第三方聚合平台查询。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现，ShipperCode=LONGBANWULIU 与状态文案映射需实网验证（单号多为 166/169 开头 12 位数字，可与 uc 规则区分）。
 */
final class Longbang implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'LONGBANWULIU';

    /** 天地华宇状态关键词 => 统一状态（以 AcceptStation 内容匹配，具体在前） */
    ...
    {
        $endpoint = (string) $this->config->get('longbang.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('longbang.ebusiness_id');
        $appKey = (string) $this->config->get('longbang.app_key');
        $customerName = (string) $this->config->get('longbang.customer_name', '');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[LONGBANG %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[LONGBANG %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[LONGBANG] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('longbang createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('longbang createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('longbang subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[LONGBANG] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[LONGBANG] %s', $message));
    }
}
