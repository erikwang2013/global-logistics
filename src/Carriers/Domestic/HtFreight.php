...
/**
 * 百世快运（百世集团旗下零担快运，区别于百世快递 HTKY，快递鸟编码 BTWL）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=BTWL）：
 * https://www.kdniao.com/api-track ；百世供应链开放平台未检索到快运轨迹公开 API 文档，
 * 以快递鸟聚合协议为准。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现；ShipperCode=BTWL 为快递鸟编码表取值，单号多为纯数字（与百世快递 HTKY 单号
 * 形态相近），生产环境是否需 CustomerName 需实网验证。
 */
final class HtFreight implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'BTWL';

    /** 百世快运状态关键词 => 统一状态（以 AcceptStation 内容匹配，具体在前） */
    ...
    {
        $endpoint = (string) $this->config->get('ht-freight.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('ht-freight.ebusiness_id');
        $appKey = (string) $this->config->get('ht-freight.app_key');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[HT-FREIGHT %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[HT-FREIGHT %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[HT-FREIGHT] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('ht-freight createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('ht-freight createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('ht-freight subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[HT-FREIGHT] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[HT-FREIGHT] %s', $message));
    }
}
