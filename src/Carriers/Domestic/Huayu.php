...
/**
 * 天地华宇物流（快递鸟编码 HOAU）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=HOAU）：
 * https://www.kdniao.com/api-track ；天地华宇官网（hoau.net）未检索到公开开放 API，
 * 仅可通过第三方聚合平台查询。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现，ShipperCode=HOAU 与状态文案映射需实网验证（单号格式存在 8 位纯数字与
 * TY+8 位数字两种说法，建议显式调用）。
 */
final class Huayu implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'HOAU';
    ...
    {
        $endpoint = (string) $this->config->get('huayu.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('huayu.ebusiness_id');
        $appKey = (string) $this->config->get('huayu.app_key');
        $customerName = (string) $this->config->get('huayu.customer_name', '');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[HUAYU %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[HUAYU %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[HUAYU] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('huayu createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('huayu createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('huayu subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[HUAYU] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[HUAYU] %s', $message));
    }
}
