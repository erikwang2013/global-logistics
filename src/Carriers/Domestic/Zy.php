...
/**
 * 增益速递（华南区域型快递，快递鸟编码 ZY）适配器。
 *
 * 契约基于快递鸟（KDNiao）即时查询协议（RequestType=1002，ShipperCode=ZY）：
 * https://www.kdniao.com/api-track ；增益速递未检索到官网开放平台 API 文档，
 * 以快递鸟聚合协议为准。
 *
 * VERIFIED-REQUIRED: 端点/签名字段（base64(md5(RequestData+AppKey, true))）按快递鸟公开
 * 协议实现；ShipperCode=ZY 为快递鸟编码表推断取值（编码表未公开列出增益速递，需实网验证），
 * 单号为纯数字（12 位左右，与 uc 的 12 位规则可能冲突，建议显式调用）。
 */
final class Zy implements CarrierInterface
{
    private const ENDPOINT = 'https://api.kdniao.com/Ebusiness/EbusinessOrderHandle.aspx';

    private const SHIPPER_CODE = 'ZY';
    ...
    {
        $endpoint = (string) $this->config->get('zy.endpoint', self::ENDPOINT);
        $eBusinessId = (string) $this->config->get('zy.ebusiness_id');
        $appKey = (string) $this->config->get('zy.app_key');
        ...
        if ($statusCode === 401 || $statusCode === 403) {
            throw new AuthException(sprintf('[ZY %d] 认证失败', $statusCode));
        }
        if ($statusCode >= 400) {
            throw new LogisticsException(sprintf('[ZY %d] 接口错误', $statusCode));
        }

        $result = json_decode((string) $response->getBody(), true);

        if (!is_array($result)) {
            throw new LogisticsException('[ZY] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('zy createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('zy createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('zy subscribe 待实现');
    }
    ...
    private function throwForApiError(string $message): never
    {
        foreach (['签名', '授权', '商户ID', 'EBusinessID', '单量'] as $keyword) {
            if (str_contains($message, $keyword)) {
                throw new AuthException(sprintf('[ZY] %s', $message));
            }
        }

        throw new LogisticsException(sprintf('[ZY] %s', $message));
    }
}
