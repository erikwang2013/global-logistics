...
/**
 * Afghanistan Post（阿富汗邮政）国际件查询（官方公开跟踪 Web 页面，GET + trackingNo
 * 参数，无认证；未开放公开 JSON 端点，故按页面表格解析：
 * Date | Postal Unit | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方公开跟踪页面
 * https://afghanpost.gov.af/en/track），需实网验证（查询参数名与表格列数以官网实际输出为准；
 * 日期为纯日期格式无时间部分，本适配器按当日零点处理；单号无效时页面无
 * 数据行，按无事件处理；状态关键词为Afghanistan Post官网普什图语/达利语/英语标准描述；
 * 无法核实的字段按 UNKNOWN/无事件兜底处理）。
 * 文档: https://afghanpost.gov.af
 */
final class AfghanistanPost implements CarrierInterface
{
    private const ENDPOINT = 'https://afghanpost.gov.af/en/track';
    ...
    private const STATUS_MAP = [
        ...
        'delivered|تحویل شد' => TrackStatus::DELIVERED,
        'returned|return to sender|بازگشت' => TrackStatus::RETURNED,
        'undeliver|failed|held|unclaimed|exception' => TrackStatus::EXCEPTION,
        'accepted|received|collected|picked up|قبول شد' => TrackStatus::PENDING,
        'in transit|transit|arrived|departed|sorted|dispatched|processed|registered|ارسال شد' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('afghanistan-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[%s %s] 认证失败', 'AFGHANISTAN-POST', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[%s %s] 接口错误', 'AFGHANISTAN-POST', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[AFGHANISTAN-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('afghanistan-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('afghanistan-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('afghanistan-post subscribe 待实现');
    }
    ...
    private function extractRows(string $html): array
    {
        ...
        foreach ($rowMatches as $rowMatch) {
            ...
            if (in_array($dateLower, ['date', 'تاریخ'], true)) {
                continue; // 表头行
            }
            $rows[] = isset($cells[3])
    ...
    private function parseTime(mixed $value): ?\DateTimeImmutable
    {
        ...
        foreach (['d/m/Y H:i:s', 'd/m/Y H:i', 'Y-m-d H:i:s', '!d/m/Y', '!Y-m-d'] as $format) {
