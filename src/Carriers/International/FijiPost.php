...
/**
 * Fiji Post（斐济邮政）国际件查询（官方公开跟踪 Web 页面，GET + trackingNo
 * 参数，无认证；未开放公开 JSON 端点，故按页面表格解析：
 * Date | Postal Unit | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方公开跟踪页面
 * https://www.fijipost.com.fj/），需实网验证（查询参数名与表格列数以官网实际输出为准；
 * 日期为纯日期格式无时间部分，本适配器按当日零点处理；单号无效时页面无
 * 数据行，按无事件处理；状态关键词为Fiji Post官网英文标准描述；
 * 无法核实的字段按 UNKNOWN/无事件兜底处理）。
 * 文档: https://www.fijipost.com.fj
 */
final class FijiPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.fijipost.com.fj/';
    ...
    {
        $endpoint = (string) $this->config->get('fiji-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[%s %s] 认证失败', 'FIJI-POST', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[%s %s] 接口错误', 'FIJI-POST', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[FIJI-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('fiji-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('fiji-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('fiji-post subscribe 待实现');
    }
