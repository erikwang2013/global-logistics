...
/**
 * Liechtensteinische Post（列支敦士登邮政）国际件查询（官方公开跟踪页面，GET + barcode 参数，无认证；
 * 官方站点跟踪页为动态页面，未开放公开 JSON 端点，故按页面表格解析：
 * Date | [Postal Unit] | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方站点 https://www.post.li/），需实网验证
 * （跟踪页路径与查询参数名以官网实际输出为准；日期为 d.m.Y 无时间部分，本适配器按当日零点处理；单号无效时页面
 * 无数据行，按无事件处理；状态关键词为Liechtensteinische Post官网德语标准描述）。
 * 文档: https://www.post.li/
 */
final class LiechtensteinPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.post.li/tracking';

    /**
     * 事件描述关键词 => 统一状态（小写匹配，`|` 分隔同义关键词，含德语）。
     * 'Zustellung'（派送中）须先于 'Zugestellt'（已投递）。
     */
    private const STATUS_MAP = [
        'out for delivery|zustellung|in zustellung|voraussichtliche zustellung' => TrackStatus::OUT_FOR_DELIVERY,
        'delivered|zugestellt|zugestellt an empfänger' => TrackStatus::DELIVERED,
        'return|rücksendung|retoure|zurück an absender' => TrackStatus::RETURNED,
        'failed|held|undeliver|nicht zustellbar|nicht abgeholt|abgelaufen|beschädigt' => TrackStatus::EXCEPTION,
        'accepted|received|aufgegeben|angenommen|collected|picked up|eingeliefert|abholbereit' => TrackStatus::PENDING,
        'in transit|transit|in transport|transportiert|arrived|eingetroffen|departed|abgefahren|sorted|sortiert|dispatched|versandt|processed|verarbeitet|registered|registriert|bearbeitet' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('liechtenstein-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[LIECHTENSTEIN-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[LIECHTENSTEIN-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[LIECHTENSTEIN-POST] 响应解析失败');
        }
        ...
        return new Tracking(
            carrierCode: 'liechtenstein-post',
    ...
    {
        throw new LogisticsException('liechtenstein-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('liechtenstein-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('liechtenstein-post subscribe 待实现');
    }
