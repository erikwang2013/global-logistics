...
/**
 * Bangladesh Post（孟加拉邮政）国际件查询（官方公开跟踪 Web 页面，GET + trackingNo
 * 参数，无认证；未开放公开 JSON 端点，故按页面表格解析：
 * Date | Postal Unit | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方公开跟踪页面
 * https://www.bdpost.gov.bd/Home/Tracking），需实网验证（查询参数名与表格列数以官网实际输出为准；
 * 日期为纯日期格式无时间部分，本适配器按当日零点处理；单号无效时页面无
 * 数据行，按无事件处理；状态关键词为Bangladesh Post官网英文标准描述；
 * 无法核实的字段按 UNKNOWN/无事件兜底处理）。
 * 文档: https://www.bdpost.gov.bd
 */
final class BangladeshPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.bdpost.gov.bd/Home/Tracking';
    ...
    private const STATUS_MAP = [
        ...
        'delivered|delivered to addressee' => TrackStatus::DELIVERED,
        'returned|return to sender' => TrackStatus::RETURNED,
        'undeliver|failed|held|unclaimed|refused|exception' => TrackStatus::EXCEPTION,
        'accepted|received|collected|picked up|ready for pickup|booking confirmed' => TrackStatus::PENDING,
        'in transit|transit|arrived|departed|sorted|dispatched|forwarded|processed|registered|on the way|sent' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('bangladesh-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[%s %s] 认证失败', 'BANGLADESH-POST', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[%s %s] 接口错误', 'BANGLADESH-POST', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[BANGLADESH-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('bangladesh-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('bangladesh-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('bangladesh-post subscribe 待实现');
    }
    ...
    private function extractRows(string $html): array
    {
        ...
        foreach ($rowMatches as $rowMatch) {
            ...
            if (in_array($dateLower, ['date'], true)) {
                continue; // 表头行
            }
            $rows[] = isset($cells[3])
    ...
    private function parseTime(mixed $value): ?\DateTimeImmutable
    {
        ...
        foreach (['d/m/Y H:i:s', 'd/m/Y H:i', 'Y-m-d\TH:i:s', 'Y-m-d H:i:s', 'Y-m-d H:i', '!d/m/Y', '!Y-m-d'] as $format) {
