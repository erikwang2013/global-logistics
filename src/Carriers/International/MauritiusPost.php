...
/**
 * Mauritius Post（毛里求斯邮政） 国际件查询（官方公开跟踪 Web 页面，GET + trackingnumber 参数，无认证；未确认
 * 公开 JSON 端点，故按页面表格解析：Date | Location | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方跟踪页 https://www.mauritiuspost.mu/），需实网验证
 * （查询参数名与表格列数以官网实际输出为准；日期为 d/m/Y 无时间部分，本适配器按
 * 当日零点处理；单号无效时页面无数据行，按无事件处理；状态关键词为官网
 * 英语 标准描述；mauritius-post.key 已预留，配置后经 X-Api-Key 头透传）。
 * 文档: https://www.mauritiuspost.mu/
 */
final class MauritiusPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.mauritiuspost.mu/';
    ...
    private const STATUS_MAP = [
        'out for delivery|for delivery|delivery to addressee' => TrackStatus::OUT_FOR_DELIVERY,
        'delivered' => TrackStatus::DELIVERED,
        'returned|return to sender|return' => TrackStatus::RETURNED,
        'failed|held|unclaimed|undeliver|refused|exception|damaged|missed|abandoned' => TrackStatus::EXCEPTION,
        'ready for pickup|available for pickup|collected|picked up|received for mailing|accepted|created|booked|waiting' => TrackStatus::PENDING,
        'in transit|transit|received|arrived|departed|sorted|dispatched|forwarded|processed|registered|shipped|handed|on the way|loaded|unloaded|imported|exported|cleared|customs|in transport|transfer' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('mauritius-post.endpoint', self::ENDPOINT);
        $key = (string) $this->config->get('mauritius-post.key');
        $headers = [
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[MAURITIUS-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[MAURITIUS-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[MAURITIUS-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('mauritius-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('mauritius-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('mauritius-post subscribe 待实现');
    }
