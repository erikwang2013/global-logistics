...
/**
 * Poste San Marino（圣马力诺邮政）国际件查询（官方公开跟踪页面，GET + barcode 参数，无认证；
 * 官方站点跟踪页为动态页面，未开放公开 JSON 端点，故按页面表格解析：
 * Date | [Postal Unit] | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方站点 https://www.poste.sm/），需实网验证
 * （跟踪页路径与查询参数名以官网实际输出为准；日期为 d/m/Y 无时间部分，本适配器按当日零点处理；单号无效时页面
 * 无数据行，按无事件处理；状态关键词为Poste San Marino官网意大利语标准描述）。
 * 文档: https://www.poste.sm/
 */
final class SanMarinoPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.poste.sm/tracking';

    /**
     * 事件描述关键词 => 统一状态（小写匹配，`|` 分隔同义关键词，含意大利语）。
     * 'In consegna'（派送中）须先于 'Consegnato'（已投递）。
     */
    private const STATUS_MAP = [
        'out for delivery|in consegna|in distribuzione|in recapito' => TrackStatus::OUT_FOR_DELIVERY,
        'delivered|consegnato|consegnata|recapitato' => TrackStatus::DELIVERED,
        'return|restituito|ritorno al mittente|resa al mittente' => TrackStatus::RETURNED,
        'failed|non consegnato|held|undeliver|rifiutato|non ritirato' => TrackStatus::EXCEPTION,
        'accepted|received|accettato|preso in carico|collected|ritirato|picked up|pronto per il ritiro|in giacenza' => TrackStatus::PENDING,
        'in transit|transit|in transito|arrived|arrivato|departed|partito|sorted|smistato|dispatched|spedito|processed|registrato|registered|in lavorazione' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('san-marino-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[SAN-MARINO-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[SAN-MARINO-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[SAN-MARINO-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('san-marino-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('san-marino-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('san-marino-post subscribe 待实现');
    }
