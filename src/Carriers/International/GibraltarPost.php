...
/**
 * Royal Gibraltar Post（直布罗陀邮政）国际件查询（官方公开跟踪页面，GET + barcode 参数，无认证；
 * 官方站点跟踪页为动态页面，未开放公开 JSON 端点，故按页面表格解析：
 * Date | [Postal Unit] | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方站点 https://www.royalgibraltarpost.gi/），需实网验证
 * （跟踪页路径与查询参数名以官网实际输出为准；日期为 d/m/Y 无时间部分，本适配器按当日零点处理；单号无效时页面
 * 无数据行，按无事件处理；状态关键词为Royal Gibraltar Post官网英语标准描述）。
 * 文档: https://www.royalgibraltarpost.gi/
 */
final class GibraltarPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.royalgibraltarpost.gi/tracking';

    /**
     * 事件描述关键词 => 统一状态（小写匹配，`|` 分隔同义关键词，含英语）。
     * 'Out for delivery'（派送中）须先于 'Delivered'（已投递）。
     */
    private const STATUS_MAP = [
        'out for delivery|on the way to delivery' => TrackStatus::OUT_FOR_DELIVERY,
        'delivered' => TrackStatus::DELIVERED,
        'returned|return to sender' => TrackStatus::RETURNED,
        'failed|held|undeliver|unclaimed|refused|exception|missing' => TrackStatus::EXCEPTION,
        'accepted|received|collected|picked up|ready for pickup|awaiting|posted' => TrackStatus::PENDING,
        'in transit|transit|arrived|departed|sorted|dispatched|processed|forwarded|registered|sent|arriving' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('gibraltar-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[GIBRALTAR-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[GIBRALTAR-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[GIBRALTAR-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('gibraltar-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('gibraltar-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('gibraltar-post subscribe 待实现');
    }
