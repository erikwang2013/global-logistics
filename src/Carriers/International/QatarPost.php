...
/**
 * Qatar Post（卡塔尔邮政） 国际件查询（官方公开跟踪 Web 页面，GET + trackingnumber 参数，无认证；未确认
 * 公开 JSON 端点，故按页面表格解析：Date | Location | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方跟踪页 https://www.qatarpost.com.qa/），需实网验证
 * （查询参数名与表格列数以官网实际输出为准；日期为 d/m/Y 无时间部分，本适配器按
 * 当日零点处理；单号无效时页面无数据行，按无事件处理；状态关键词为官网
 * 英语/阿拉伯语 标准描述；qatar-post.key 已预留，配置后经 X-Api-Key 头透传）。
 * 文档: https://www.qatarpost.com.qa/
 */
final class QatarPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.qatarpost.com.qa/';
    ...
    {
        $endpoint = (string) $this->config->get('qatar-post.endpoint', self::ENDPOINT);
        $key = (string) $this->config->get('qatar-post.key');
        $headers = [
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[QATAR-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[QATAR-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[QATAR-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('qatar-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('qatar-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('qatar-post subscribe 待实现');
    }
