...
/**
 * Jersey Post（泽西邮政）国际件查询（官方公开跟踪页面，GET + barcode 参数，无认证；
 * 官方站点跟踪页为动态页面，未开放公开 JSON 端点，故按页面表格解析：
 * Date | [Postal Unit] | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方站点 https://www.jerseypost.com/），需实网验证
 * （跟踪页路径与查询参数名以官网实际输出为准；日期为 d/m/Y 无时间部分，本适配器按当日零点处理；单号无效时页面
 * 无数据行，按无事件处理；状态关键词为Jersey Post官网英语标准描述）。
 * 文档: https://www.jerseypost.com/
 */
final class JerseyPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.jerseypost.com/track-an-item';
    ...
    {
        $endpoint = (string) $this->config->get('jersey-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[JERSEY-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[JERSEY-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[JERSEY-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('jersey-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('jersey-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('jersey-post subscribe 待实现');
    }
