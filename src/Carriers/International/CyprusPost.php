...
/**
 * Cyprus Post（塞浦路斯邮政）国际件查询（官方 Track & Trace 结果页，GET + barcode
 * 参数，无认证；官方 API 需经官网联系表单申请，未公开 JSON 端点，故按页面
 * 表格解析：Date | Postal Unit | Description 或 Date | Postal Unit | City |
 * Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方 Track & Trace 页面
 * https://www.cypruspost.post/en/track-n-trace-results，barcode 为 UPU S10 格式
 * 如 RR123456789CY），需实网验证（结果页查询参数名与表格列数以官网实际输出
 * 为准；日期无时间部分，本适配器按当日零点处理；单号无效时页面无数据行，
 * 按无事件处理；状态关键词为 Cyprus Post 官网英文标准描述）。
 * 文档: https://www.cypruspost.post/en/track-n-trace-results
 */
final class CyprusPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.cypruspost.post/en/track-n-trace-results';
    ...
    private const STATUS_MAP = [
        ...
        'delivered|successful delivery' => TrackStatus::DELIVERED,
        ...
        'posted|accepted|received|collected|picked up|ready for pickup' => TrackStatus::PENDING,
        'in transit|transit|processed|arrived|departed|sorted|dispatched|forwarded|transferred|on its way|forwarded for processing' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('cyprus-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[CYPRUS-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[CYPRUS-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[CYPRUS-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('cyprus-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('cyprus-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('cyprus-post subscribe 待实现');
    }

    /**
     * 从结果页 HTML 中提取表格行（Date | Postal Unit | [City] | Description，
    ...
    private function extractRows(string $html): array
    {
        ...
        foreach ($rowMatches as $rowMatch) {
            ...
            $date = $cells[0];
            if (in_array(strtolower($date), ['date', 'ημερομηνία', 'датум', 'datum', 'data', 'дата'], true)) {
                continue; // 表头行
            }
            $rows[] = isset($cells[3])
