...
/**
 * Correios de Moçambique（莫桑比克邮政） 国际件查询（官方公开跟踪 Web 页面，GET + codigo 参数，无认证；未确认
 * 公开 JSON 端点，故按页面表格解析：Date | Location | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方跟踪页 https://www.correios.co.mz/），需实网验证
 * （查询参数名与表格列数以官网实际输出为准；日期为 d/m/Y 无时间部分，本适配器按
 * 当日零点处理；单号无效时页面无数据行，按无事件处理；状态关键词为官网
 * 葡萄牙语 标准描述；mozambique-post.key 已预留，配置后经 X-Api-Key 头透传）。
 * 文档: https://www.correios.co.mz/
 */
final class MozambiquePost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.correios.co.mz/';
    ...
    private const STATUS_MAP = [
        'out for delivery|em distribuição|saída para entrega|distribuição|em rota de entrega' => TrackStatus::OUT_FOR_DELIVERY,
        'delivered|entregue ao destinatário|entregue|entrega efectuada|entrega efetuada' => TrackStatus::DELIVERED,
        'devolvid|devolução|retorno|return' => TrackStatus::RETURNED,
        'recusad|avariad|não reclam|falha|retenç|endereço incorrecto|endereço incorreto|failed|held|unclaimed|undeliver|exception' => TrackStatus::EXCEPTION,
        'aguarda levantamento|em armazém|disponível para|pronto para|collected|picked up|aceite|rececionado|recepcionado' => TrackStatus::PENDING,
        'em trânsito|em transito|transit|recebido|recebida|chegou|partiu|expedid|despachado|triagem|importaç|exportaç|processado|registado|encaminhado|carregado|descarregado|in transit|in_transit' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('mozambique-post.endpoint', self::ENDPOINT);
        $key = (string) $this->config->get('mozambique-post.key');
        $headers = [
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[MOZAMBIQUE-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[MOZAMBIQUE-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[MOZAMBIQUE-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('mozambique-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('mozambique-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('mozambique-post subscribe 待实现');
    }
