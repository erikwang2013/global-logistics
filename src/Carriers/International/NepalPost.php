...
/**
 * Nepal Post（尼泊尔邮政）国际件查询（官方公开跟踪 Web 页面，GET + trackingNo
 * 参数，无认证；未开放公开 JSON 端点，故按页面表格解析：
 * Date | Postal Unit | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方公开跟踪页面
 * https://www.nepalpost.gov.np/postal-services/parcel-tracking），需实网验证（查询参数名与表格列数以官网实际输出为准；
 * 日期为纯日期格式无时间部分，本适配器按当日零点处理；单号无效时页面无
 * 数据行，按无事件处理；状态关键词为Nepal Post官网英文标准描述；
 * 无法核实的字段按 UNKNOWN/无事件兜底处理）。
 * 文档: https://www.nepalpost.gov.np
 */
final class NepalPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.nepalpost.gov.np/postal-services/parcel-tracking';
    ...
    private const STATUS_MAP = [
        ...
        'accepted|received|collected|picked up|ready for pickup' => TrackStatus::PENDING,
        'in transit|transit|arrived|departed|sorted|dispatched|forwarded|processed|registered|on the way' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('nepal-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[%s %s] 认证失败', 'NEPAL-POST', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[%s %s] 接口错误', 'NEPAL-POST', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[NEPAL-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('nepal-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('nepal-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('nepal-post subscribe 待实现');
    }
    ...
    private function extractRows(string $html): array
    {
        ...
        foreach ($rowMatches as $rowMatch) {
            ...
            if (in_array($dateLower, ['date', 'मिति'], true)) {
                continue; // 表头行
            }
            $rows[] = isset($cells[3])
    ...
    private function parseTime(mixed $value): ?\DateTimeImmutable
    {
        ...
        foreach (['Y-m-d H:i:s', 'Y-m-d H:i', 'Y-m-d\TH:i:s', 'd/m/Y H:i:s', '!Y-m-d', '!d/m/Y'] as $format) {
