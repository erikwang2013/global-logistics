...
/**
 * Poste Vaticane（梵蒂冈邮政）国际件查询（官方公开跟踪页面，GET + barcode 参数，无认证；
 * 官方站点跟踪页为动态页面，未开放公开 JSON 端点，故按页面表格解析：
 * Date | [Postal Unit] | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方站点 https://www.vaticanposte.va/），需实网验证
 * （跟踪页路径与查询参数名以官网实际输出为准；日期为 d/m/Y 无时间部分，本适配器按当日零点处理；单号无效时页面
 * 无数据行，按无事件处理；状态关键词为Poste Vaticane官网意大利语标准描述）。
 * 文档: https://www.vaticanposte.va/
 */
final class VaticanPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.vaticanposte.va/tracking';
    ...
    {
        $endpoint = (string) $this->config->get('vatican-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[VATICAN-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[VATICAN-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[VATICAN-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('vatican-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('vatican-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('vatican-post subscribe 待实现');
    }
