...
/**
 * Correos Andorra（安道尔邮政）国际件查询（官方公开跟踪页面，GET + barcode 参数，无认证；
 * 官方站点跟踪页为动态页面，未开放公开 JSON 端点，故按页面表格解析：
 * Date | [Postal Unit] | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方站点 https://www.correosandorra.ad/），需实网验证
 * （跟踪页路径与查询参数名以官网实际输出为准；日期为 d/m/Y 无时间部分，本适配器按当日零点处理；单号无效时页面
 * 无数据行，按无事件处理；状态关键词为Correos Andorra官网加泰罗尼亚语/西班牙语标准描述）。
 * 文档: https://www.correosandorra.ad/
 */
final class AndorraPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.correosandorra.ad/tracking';

    /**
     * 事件描述关键词 => 统一状态（小写匹配，`|` 分隔同义关键词，含加泰罗尼亚语/西班牙语）。
     * 'En repartiment'（派送中）须先于 'Lliurat'（已投递）。
     */
    private const STATUS_MAP = [
        'out for delivery|repartiment|reparto|repartint' => TrackStatus::OUT_FOR_DELIVERY,
        'delivered|lliurat|lliurada|entregado|entregada' => TrackStatus::DELIVERED,
        'return|retorn|retorno|devolució|devolución' => TrackStatus::RETURNED,
        'failed|held|undeliver|no lliurat|no entregado|refusat|rechazado|no reclamat' => TrackStatus::EXCEPTION,
        'accepted|received|admissió|admisión|acceptat|recollit|picked up' => TrackStatus::PENDING,
        'in transit|transit|en trànsit|en tránsito|arrived|arribat|llegado|departed|sortit|salido|sorted|classificat|dispatched|enviat|enviado|registered|registrat|processat' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('andorra-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[ANDORRA-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[ANDORRA-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[ANDORRA-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('andorra-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('andorra-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('andorra-post subscribe 待实现');
    }
