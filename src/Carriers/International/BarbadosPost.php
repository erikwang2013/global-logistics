...
/**
 * Barbados Post（巴巴多斯邮政）国际件查询（官方公开跟踪 Web 页面，GET + trackingNo 参数，
 * 无认证；未开放公开 JSON 端点，故按页面表格解析：Date | Office | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方跟踪页面 https://barbadospostal.com/），需实网验证
 * （查询参数名与表格列数以官网实际输出为准；日期无时间部分时按当日零点处理；
 * 单号无效时页面无数据行，按无事件处理；状态关键词为官网西班牙语/英语标准描述，
 * 查不到的字段按 IN_TRANSIT 兜底）。
 * 文档: https://barbadospostal.com/
 */
final class BarbadosPost implements CarrierInterface
{
    private const ENDPOINT = 'https://barbadospostal.com/';
    ...
    {
        $endpoint = (string) $this->config->get('barbados-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[BARBADOS-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[BARBADOS-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[BARBADOS-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('barbados-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('barbados-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('barbados-post subscribe 待实现');
    }
