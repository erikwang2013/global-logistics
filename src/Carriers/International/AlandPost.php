...
/**
 * Posten Åland（奥兰群岛邮政）国际件查询（官方公开跟踪页面，GET + barcode 参数，无认证；
 * 官方站点跟踪页为动态页面，未开放公开 JSON 端点，故按页面表格解析：
 * Date | [Postal Unit] | [City] | Description）。
 *
 * VERIFIED-REQUIRED: 契约基于公开网页资料（官方站点 https://www.posten.ax/），需实网验证
 * （跟踪页路径与查询参数名以官网实际输出为准；日期为 Y-m-d 无时间部分，本适配器按当日零点处理；单号无效时页面
 * 无数据行，按无事件处理；状态关键词为Posten Åland官网瑞典语标准描述）。
 * 文档: https://www.posten.ax/
 */
final class AlandPost implements CarrierInterface
{
    private const ENDPOINT = 'https://www.posten.ax/tracking';

    /**
     * 事件描述关键词 => 统一状态（小写匹配，`|` 分隔同义关键词，含瑞典语）。
     * 'Utkörning'（派送中）须先于 'Levererad'（已投递）。
     */
    private const STATUS_MAP = [
        'out for delivery|utkörning|till utdelning|i utdelning|utdelning pågår' => TrackStatus::OUT_FOR_DELIVERY,
        'delivered|levererad|utdelad|avhämtad' => TrackStatus::DELIVERED,
        'return|returnerad|retur' => TrackStatus::RETURNED,
        'failed|misslyckad|held|undeliver|inte avhämtad|avvisad|skadad' => TrackStatus::EXCEPTION,
        'accepted|received|mottagen|inlämnad|accepterad|collected|upphämtad|picked up|finns att hämta' => TrackStatus::PENDING,
        'in transit|transit|i transit|arrived|anlänt|departed|avgått|sorted|sorterad|dispatched|skickad|processed|behandlad|registered|registrerad|under transport' => TrackStatus::IN_TRANSIT,
    ];
    ...
    {
        $endpoint = (string) $this->config->get('aland-post.endpoint', self::ENDPOINT);
        ...
        if ($status === 401 || $status === 403) {
            throw new AuthException(sprintf('[ALAND-POST %s] 认证失败', $status));
        }
        if ($status >= 400) {
            throw new LogisticsException(sprintf('[ALAND-POST %s] 接口错误', $status));
        }
        ...
        if ($decoded !== null && !is_array($decoded)) {
            throw new LogisticsException('[ALAND-POST] 响应解析失败');
        }
    ...
    {
        throw new LogisticsException('aland-post createOrder 待实现');
    }

    public function createLabel(Order $order, array $options = []): Label
    {
        throw new LogisticsException('aland-post createLabel 待实现');
    }

    public function subscribe(string $callbackUrl, array $options = []): void
    {
        throw new LogisticsException('aland-post subscribe 待实现');
    }
