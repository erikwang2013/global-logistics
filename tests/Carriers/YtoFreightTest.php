...
final class YtoFreightTest extends TestCase
{
    private function makeAdapter(FakeHttpClient $http): YtoFreight
    {
        return new YtoFreight(
            new Config(['yto-freight' => [
    ...
    public function testQueryTrackParses(): void
    {
        ...
        $http->handler = function (Request $request) {
            ...
            $this->assertSame('YUANTONGKUAIYUN', $requestData['ShipperCode']);
            $this->assertSame('YT745102839612', $requestData['LogisticCode']);
            $this->assertSame(
            ...
            return new Response(200, ['Content-Type' => 'application/json'],
                file_get_contents(__DIR__ . '/../fixtures/yto-freight/track.json'));
        };

        $tracking = $this->makeAdapter($http)->queryTrack('YT745102839612');

        $this->assertSame('yto-freight', $tracking->carrierCode);
        $this->assertSame('YT745102839612', $tracking->trackingNo);
        $this->assertSame(TrackStatus::DELIVERED, $tracking->status);
        $this->assertCount(4, $tracking->events);
        $this->assertSame('圆通快运已揽收', $tracking->events[0]->description);
    ...
    public function testQueryTrackReversesDescendingEvents(): void
    {
        ...
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            json_encode([
                'EBusinessID' => '1264783',
                'Success' => true,
                'ShipperCode' => 'YUANTONGKUAIYUN',
                'State' => '3',
                'Traces' => [
                    ['AcceptTime' => '2026-08-15 10:12:00', 'AcceptStation' => '快件已签收', 'Remark' => ''],
                    ['AcceptTime' => '2026-08-12 09:30:00', 'AcceptStation' => '圆通快运已揽收', 'Remark' => ''],
                ],
    ...
    public function testQueryTrackThrowsWhenNoEvents(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/yto-freight/empty.json'));

        $this->expectException(TrackingNotFoundException::class);

        $this->makeAdapter($http)->queryTrack('YT745102839612');
    }
    ...
    public function testQueryTrackMapsBusinessError(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/yto-freight/error.json'));

        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[YTO-FREIGHT] 查无此单');
    ...
    public function testQueryTrackMapsAuthError(): void
    {
        ...
        $this->expectExceptionMessage('[YTO-FREIGHT 401] 认证失败');
    ...
    public function testQueryTrackThrowsOnInvalidJson(): void
    {
        ...
        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[YTO-FREIGHT] 响应解析失败');
    ...
    public function testUnsupportedMethodsThrow(): void
    {
        ...
        try {
            ...
            $this->assertSame('yto-freight createOrder 待实现', $e->getMessage());
        }

        try {
            $adapter->createLabel(new Order('YT745102839612'));
            $this->fail('createLabel should throw');
        } catch (LogisticsException $e) {
            $this->assertSame('yto-freight createLabel 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('yto-freight subscribe 待实现', $e->getMessage());
        }
    }
}
