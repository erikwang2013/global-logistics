...
final class CaeTest extends TestCase
{
    private function makeAdapter(FakeHttpClient $http): Cae
    {
        return new Cae(
            new Config(['cae' => [
    ...
    public function testQueryTrackParses(): void
    {
        ...
        $http->handler = function (Request $request) {
            ...
            $this->assertSame('CAE', $requestData['ShipperCode']);
            $this->assertSame('CA345678901234', $requestData['LogisticCode']);
            $this->assertSame(
            ...
            return new Response(200, ['Content-Type' => 'application/json'],
                file_get_contents(__DIR__ . '/../fixtures/cae/track.json'));
        };

        $tracking = $this->makeAdapter($http)->queryTrack('CA345678901234');

        $this->assertSame('cae', $tracking->carrierCode);
        $this->assertSame('CA345678901234', $tracking->trackingNo);
        $this->assertSame(TrackStatus::DELIVERED, $tracking->status);
        $this->assertCount(4, $tracking->events);
        $this->assertSame('民航快递已揽收', $tracking->events[0]->description);
        ...
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            json_encode([
                ...
                'Traces' => [
                    ...
                    ['AcceptTime' => '2026-08-12 09:30:00', 'AcceptStation' => '民航快递已揽收', 'Remark' => ''],
                ],
    ...
    public function testQueryTrackThrowsWhenNoEvents(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/cae/empty.json'));

        $this->expectException(TrackingNotFoundException::class);

        $this->makeAdapter($http)->queryTrack('CA345678901234');
    }
    ...
    public function testQueryTrackMapsBusinessError(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/cae/error.json'));

        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[CAE] 查无此单');
    ...
    public function testQueryTrackMapsAuthError(): void
    {
        ...
        $this->expectExceptionMessage('[CAE 401] 认证失败');
    ...
    public function testQueryTrackThrowsOnInvalidJson(): void
    {
        ...
        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[CAE] 响应解析失败');
    ...
    public function testUnsupportedMethodsThrow(): void
    {
        ...
        try {
            ...
            $this->assertSame('cae createOrder 待实现', $e->getMessage());
        }

        try {
            $adapter->createLabel(new Order('CA345678901234'));
            $this->fail('createLabel should throw');
        } catch (LogisticsException $e) {
            $this->assertSame('cae createLabel 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('cae subscribe 待实现', $e->getMessage());
        }
    }
}
