...
final class JiajiTest extends TestCase
{
    private function makeAdapter(FakeHttpClient $http): Jiaji
    {
        return new Jiaji(
            new Config(['jiaji' => [
    ...
    public function testQueryTrackParses(): void
    {
        ...
        $http->handler = function (Request $request) {
            ...
            $this->assertSame('JIAJIKUAIDI', $requestData['ShipperCode']);
            $this->assertSame('123456789', $requestData['LogisticCode']);
            ...
            return new Response(200, ['Content-Type' => 'application/json'],
                file_get_contents(__DIR__ . '/../fixtures/jiaji/track.json'));
        };

        $tracking = $this->makeAdapter($http)->queryTrack('123456789');

        $this->assertSame('jiaji', $tracking->carrierCode);
        $this->assertSame('123456789', $tracking->trackingNo);
        $this->assertSame(TrackStatus::DELIVERED, $tracking->status);
        $this->assertCount(4, $tracking->events);
        $this->assertSame('佳吉快运已揽收', $tracking->events[0]->description);
    ...
    public function testQueryTrackReversesDescendingEvents(): void
    {
        ...
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            json_encode([
                'EBusinessID' => '1264783',
                'Success' => true,
                'ShipperCode' => 'JIAJIKUAIDI',
                'State' => '3',
                'Traces' => [
                    ['AcceptTime' => '2026-08-15 10:12:00', 'AcceptStation' => '快件已签收', 'Remark' => ''],
                    ['AcceptTime' => '2026-08-12 09:30:00', 'AcceptStation' => '佳吉快运已揽收', 'Remark' => ''],
                ],
    ...
    public function testQueryTrackThrowsWhenNoEvents(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/jiaji/empty.json'));

        $this->expectException(TrackingNotFoundException::class);

        $this->makeAdapter($http)->queryTrack('123456789');
    }
    ...
    public function testQueryTrackMapsBusinessError(): void
    {
        ...
        $http->handler = fn (Request $request) => new Response(400, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/jiaji/error.json'));

        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[JIAJI 400] 接口错误');
    ...
    public function testQueryTrackMapsAuthError(): void
    {
        ...
        $this->expectExceptionMessage('[JIAJI 401] 认证失败');
    ...
    public function testQueryTrackThrowsOnInvalidJson(): void
    {
        ...
        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[JIAJI] 响应解析失败');
    ...
    public function testUnsupportedMethodsThrow(): void
    {
        ...
        try {
            ...
            $this->assertSame('jiaji createOrder 待实现', $e->getMessage());
        }

        try {
            $adapter->createLabel(new Order('123456789'));
            $this->fail('createLabel should throw');
        } catch (LogisticsException $e) {
            $this->assertSame('jiaji createLabel 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('jiaji subscribe 待实现', $e->getMessage());
        }
    }
}
