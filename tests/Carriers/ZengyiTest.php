...
final class ZengyiTest extends TestCase
{
    private function makeAdapter(FakeHttpClient $http): Zengyi
    {
        return new Zengyi(
            new Config(['zengyi' => [
    ...
    public function testQueryTrackParses(): void
    {
        ...
        $http->handler = function (Request $request) {
            ...
            $this->assertSame('ZENY', $requestData['ShipperCode']);
            ...
            return new Response(200, ['Content-Type' => 'application/json'],
                file_get_contents(__DIR__ . '/../fixtures/zengyi/track.json'));
        };

        $tracking = $this->makeAdapter($http)->queryTrack('1234567890');

        $this->assertSame('zengyi', $tracking->carrierCode);
    ...
    public function testQueryTrackThrowsWhenNoEvents(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/zengyi/empty.json'));
    ...
    public function testQueryTrackMapsBusinessError(): void
    {
        ...
        $http->handler = fn (Request $request) => new Response(400, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/zengyi/error.json'));

        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[ZENGYI 400] 接口错误');
    ...
    public function testQueryTrackMapsAuthError(): void
    {
        ...
        $this->expectExceptionMessage('[ZENGYI 401] 认证失败');
    ...
    public function testQueryTrackThrowsOnInvalidJson(): void
    {
        ...
        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[ZENGYI] 响应解析失败');
    ...
    public function testUnsupportedMethodsThrow(): void
    {
        ...
        try {
            ...
            $this->assertSame('zengyi createOrder 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('zengyi createLabel 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('zengyi subscribe 待实现', $e->getMessage());
        }
    }
}
