...
final class LongbangTest extends TestCase
{
    private function makeAdapter(FakeHttpClient $http): Longbang
    {
        return new Longbang(
            new Config(['longbang' => [
    ...
    public function testQueryTrackParses(): void
    {
        ...
        $http->handler = function (Request $request) {
            ...
            $this->assertSame('LONGBANWULIU', $requestData['ShipperCode']);
            $this->assertSame('166123456789', $requestData['LogisticCode']);
            ...
            return new Response(200, ['Content-Type' => 'application/json'],
                file_get_contents(__DIR__ . '/../fixtures/longbang/track.json'));
        };

        $tracking = $this->makeAdapter($http)->queryTrack('166123456789');

        $this->assertSame('longbang', $tracking->carrierCode);
        $this->assertSame('166123456789', $tracking->trackingNo);
        $this->assertSame(TrackStatus::DELIVERED, $tracking->status);
        $this->assertCount(4, $tracking->events);
        $this->assertSame('龙邦速递已揽收', $tracking->events[0]->description);
        ...
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            json_encode([
                ...
                'Traces' => [
                    ...
                    ['AcceptTime' => '2026-08-12 09:30:00', 'AcceptStation' => '龙邦速递已揽收', 'Remark' => ''],
                ],
    ...
    public function testQueryTrackThrowsWhenNoEvents(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/longbang/empty.json'));

        $this->expectException(TrackingNotFoundException::class);

        $this->makeAdapter($http)->queryTrack('166123456789');
    }
    ...
    public function testQueryTrackMapsBusinessError(): void
    {
        ...
        $http->handler = fn (Request $request) => new Response(400, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/longbang/error.json'));

        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[LONGBANG 400] 接口错误');
    ...
    public function testQueryTrackMapsAuthError(): void
    {
        ...
        $this->expectExceptionMessage('[LONGBANG 401] 认证失败');
    ...
    public function testQueryTrackThrowsOnInvalidJson(): void
    {
        ...
        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[LONGBANG] 响应解析失败');
    ...
    public function testUnsupportedMethodsThrow(): void
    {
        ...
        try {
            ...
            $this->assertSame('longbang createOrder 待实现', $e->getMessage());
        }

        try {
            $adapter->createLabel(new Order('166123456789'));
            $this->fail('createLabel should throw');
        } catch (LogisticsException $e) {
            $this->assertSame('longbang createLabel 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('longbang subscribe 待实现', $e->getMessage());
        }
    }
}
