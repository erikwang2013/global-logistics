...
final class FengwangTest extends TestCase
{
    private function makeAdapter(FakeHttpClient $http): Fengwang
    {
        return new Fengwang(
            new Config(['fengwang' => [
    ...
    public function testQueryTrackParses(): void
    {
        ...
        $http->handler = function (Request $request) {
            ...
            $this->assertSame('FWX', $requestData['ShipperCode']);
            $this->assertSame('SF5009345678', $requestData['LogisticCode']);
            ...
            return new Response(200, ['Content-Type' => 'application/json'],
                file_get_contents(__DIR__ . '/../fixtures/fengwang/track.json'));
        };

        $tracking = $this->makeAdapter($http)->queryTrack('SF5009345678');

        $this->assertSame('fengwang', $tracking->carrierCode);
        $this->assertSame('SF5009345678', $tracking->trackingNo);
        $this->assertSame(TrackStatus::DELIVERED, $tracking->status);
        $this->assertCount(4, $tracking->events);
        $this->assertSame('丰网速运已揽收', $tracking->events[0]->description);
        ...
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            json_encode([
                ...
                'Traces' => [
                    ...
                    ['AcceptTime' => '2026-08-12 09:30:00', 'AcceptStation' => '丰网速运已揽收', 'Remark' => ''],
                ],
    ...
    public function testQueryTrackThrowsWhenNoEvents(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/fengwang/empty.json'));

        $this->expectException(TrackingNotFoundException::class);

        $this->makeAdapter($http)->queryTrack('SF5009345678');
    }
    ...
    public function testQueryTrackMapsBusinessError(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/fengwang/error.json'));

        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[FENGWANG] 查无此单');
    ...
    public function testQueryTrackMapsAuthError(): void
    {
        ...
        $this->expectExceptionMessage('[FENGWANG 403] 认证失败');
    ...
    public function testQueryTrackThrowsOnInvalidJson(): void
    {
        ...
        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[FENGWANG] 响应解析失败');
    ...
    public function testUnsupportedMethodsThrow(): void
    {
        ...
        try {
            ...
            $this->assertSame('fengwang createOrder 待实现', $e->getMessage());
        }

        try {
            $adapter->createLabel(new Order('SF5009345678'));
            $this->fail('createLabel should throw');
        } catch (LogisticsException $e) {
            $this->assertSame('fengwang createLabel 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('fengwang subscribe 待实现', $e->getMessage());
        }
    }
}
