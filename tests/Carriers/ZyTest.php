...
final class ZyTest extends TestCase
{
    private function makeAdapter(FakeHttpClient $http): Zy
    {
        return new Zy(
            new Config(['zy' => [
    ...
    public function testQueryTrackParses(): void
    {
        ...
        $http->handler = function (Request $request) {
            ...
            $this->assertSame('ZY', $requestData['ShipperCode']);
            $this->assertSame('884097632531', $requestData['LogisticCode']);
            $this->assertSame(
            ...
            return new Response(200, ['Content-Type' => 'application/json'],
                file_get_contents(__DIR__ . '/../fixtures/zy/track.json'));
        };

        $tracking = $this->makeAdapter($http)->queryTrack('884097632531');

        $this->assertSame('zy', $tracking->carrierCode);
        $this->assertSame('884097632531', $tracking->trackingNo);
        $this->assertSame(TrackStatus::DELIVERED, $tracking->status);
        $this->assertCount(4, $tracking->events);
        $this->assertSame('增益速递已揽收', $tracking->events[0]->description);
        ...
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            json_encode([
                ...
                'Traces' => [
                    ...
                    ['AcceptTime' => '2026-08-12 09:30:00', 'AcceptStation' => '增益速递已揽收', 'Remark' => ''],
                ],
    ...
    public function testQueryTrackThrowsWhenNoEvents(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/zy/empty.json'));

        $this->expectException(TrackingNotFoundException::class);

        $this->makeAdapter($http)->queryTrack('884097632531');
    }
    ...
    public function testQueryTrackMapsBusinessError(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/zy/error.json'));

        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[ZY] 查无此单');
    ...
    public function testQueryTrackMapsAuthError(): void
    {
        ...
        $this->expectExceptionMessage('[ZY 401] 认证失败');
    ...
    public function testQueryTrackThrowsOnInvalidJson(): void
    {
        ...
        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[ZY] 响应解析失败');
    ...
    public function testUnsupportedMethodsThrow(): void
    {
        ...
        try {
            ...
            $this->assertSame('zy createOrder 待实现', $e->getMessage());
        }

        try {
            $adapter->createLabel(new Order('884097632531'));
            $this->fail('createLabel should throw');
        } catch (LogisticsException $e) {
            $this->assertSame('zy createLabel 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('zy subscribe 待实现', $e->getMessage());
        }
    }
}
