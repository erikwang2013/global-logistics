...
final class HuayuTest extends TestCase
{
    private function makeAdapter(FakeHttpClient $http): Huayu
    {
        return new Huayu(
            new Config(['huayu' => [
    ...
    public function testQueryTrackParses(): void
    {
        ...
        $http->handler = function (Request $request) {
            ...
            $this->assertSame('HOAU', $requestData['ShipperCode']);
            $this->assertSame('23784475', $requestData['LogisticCode']);
            ...
            return new Response(200, ['Content-Type' => 'application/json'],
                file_get_contents(__DIR__ . '/../fixtures/huayu/track.json'));
        };

        $tracking = $this->makeAdapter($http)->queryTrack('23784475');

        $this->assertSame('huayu', $tracking->carrierCode);
        $this->assertSame('23784475', $tracking->trackingNo);
        $this->assertSame(TrackStatus::DELIVERED, $tracking->status);
        $this->assertCount(4, $tracking->events);
        $this->assertSame('华宇物流已揽收', $tracking->events[0]->description);
        ...
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            json_encode([
                ...
                'Traces' => [
                    ...
                    ['AcceptTime' => '2026-08-12 09:30:00', 'AcceptStation' => '华宇物流已揽收', 'Remark' => ''],
                ],
    ...
    public function testQueryTrackThrowsWhenNoEvents(): void
    {
        $http = new FakeHttpClient();
        $http->handler = fn (Request $request) => new Response(200, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/huayu/empty.json'));

        $this->expectException(TrackingNotFoundException::class);

        $this->makeAdapter($http)->queryTrack('23784475');
    }
    ...
    public function testQueryTrackMapsBusinessError(): void
    {
        ...
        $http->handler = fn (Request $request) => new Response(400, ['Content-Type' => 'application/json'],
            file_get_contents(__DIR__ . '/../fixtures/huayu/error.json'));

        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[HUAYU 400] 接口错误');
    ...
    public function testQueryTrackMapsAuthError(): void
    {
        ...
        $this->expectExceptionMessage('[HUAYU 401] 认证失败');
    ...
    public function testQueryTrackThrowsOnInvalidJson(): void
    {
        ...
        $this->expectException(LogisticsException::class);
        $this->expectExceptionMessage('[HUAYU] 响应解析失败');
    ...
    public function testUnsupportedMethodsThrow(): void
    {
        ...
        try {
            ...
            $this->assertSame('huayu createOrder 待实现', $e->getMessage());
        }

        try {
            $adapter->createLabel(new Order('23784475'));
            $this->fail('createLabel should throw');
        } catch (LogisticsException $e) {
            $this->assertSame('huayu createLabel 待实现', $e->getMessage());
        }
        ...
        try {
            ...
            $this->assertSame('huayu subscribe 待实现', $e->getMessage());
        }
    }
}
